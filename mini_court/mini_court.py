...
import numpy as np
import constants
from utils import (convert_pixel_to_distance_to_meters, convert_meters_to_pixel_distance, get_foot_position,
                   get_closest_keypoint_index, get_height_of_box, measure_xy_distance, get_center_of_the_box,
                   measure_distance)
...
class MiniCourt:
	...
	def convert_pixels_to_meters(self, pixels):
		return convert_pixel_to_distance_to_meters(pixels,
		                                           constants.DOUBLE_LINE_WIDTH,
		                                           self.court_drawing_width)
	...
	def draw_court(self, frame):
		...
		cv2.circle(frame, net_start_point, 5, (120, 250, 70), -1)
		cv2.circle(frame, net_end_point, 5, (120, 250, 70), -1)
		cv2.line(frame, net_start_point, net_end_point, (120, 250, 70), 2)
	...
	def get_mini_court_coordinates(self, object_position, closest_key_point, closest_key_point_index,
		...
		# Get the distance from the cloest keypoint
		distance_from_keypoint_x_pixels, distance_from_keypoint_y_pixels = measure_xy_distance(object_position,
		                                                                                       closest_key_point)
		# Convert to meters
		distance_from_keypoint_x_to_meters = convert_pixel_to_distance_to_meters(distance_from_keypoint_x_pixels,
		...
		# Convert to pixels for mini_cort
		mini_court_x_distance_pixels = self.convert_meters_to_pixels(distance_from_keypoint_x_to_meters)
		...
		return mini_court_player_position[0], mini_court_player_position[1]
	...
	def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
		...
		for frame_num, player_box in enumerate(player_boxes):
			...
			ball_position = get_center_of_the_box(ball_box)
			closest_player_id_to_ball = min(player_box.keys(), key=lambda x: measure_distance(ball_position,
			                                                                                  get_foot_position(
				                                                                                  player_box[x])))
			...
			for player_id, box in player_box.items():
				...
				# print(f'Player Id {player_id} foot position {foot_position}')

				# Get the cloest keypoint in pixels
				closest_key_point_index = get_closest_keypoint_index(foot_position, original_court_key_points,
				                                                     [0, 2, 12, 13])
				# print(f'Frame num: {frame_num}, closest keypoint index: {closest_key_point_index}, player id: {player_id}')
				...
				bboxes_heights_in_pixels = [get_height_of_box(player_boxes[i][player_id]) for i in
				...
				# print(mini_court_player_position)
